    ...
    if(this.amdList.length > 0) {
      const loaderDec = t.variableDeclaration('const', [
        t.variableDeclarator(
          t.arrayPattern(this.amdList.map(item => t.identifier(item.specifier != null ? item.specifier.name : 'not_use_module_' + Math.random().toString(32).substr(2)))),
          t.awaitExpression(
            t.callExpression(
              t.identifier('loadModules'),
              [t.arrayExpression(this.amdList.map(item => t.stringLiteral(item.source.value)))]
            )
          )
        )
      ])
